...
const QRScannerScreen: React.FC = () => {
  ...
  const handleQRCodeScanned = async (data: string, smsCode?: string) => {
    ...
    try {
      if (!currentLocation) {
        Alert.alert('Error', 'Location permission required for attendance marking');
        setScanned(false);
        setLoading(false);
        return;
      }
      ...
      if (!/^\d{6}$/.test(otp)) {
        Alert.alert(
          'Invalid QR Code', 
          `The scanned QR code does not contain a valid OTP code.\n\nScanned data: "${data}"\nExtracted: "${otp}"`
        );
        setScanned(false);
        setLoading(false);
        return;
    ...
    <View style={styles.container}>
      ...
      <View style={styles.footer}>
        ...
        <View style={styles.buttonRow}>
          ...
          >
            <Text style={styles.flipButtonText}>📷 {facing === 'back' ? 'Front Camera' : 'Back Camera'}</Text>
          </TouchableOpacity>
          <TouchableOpacity
